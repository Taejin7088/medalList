  ...
  const handleCountryList = {
    ...
    isCountryInList: (targetCountryName) => {
      return countryList.some(
        (county) => county.countryName === targetCountryName
      );
    },
    ...
    sortByMedal: (conlist) => {
      switch (sortOption) {
        ...
        case "totalMedal":
          conlist.sort((a, b) => {
            return (
              Number(b.goldMedalCount) +
              Number(b.silverMedalCount) +
              Number(b.bronzeMedalCount) -
              (Number(a.goldMedalCount) +
                Number(a.silverMedalCount) +
                Number(a.bronzeMedalCount))
            );
          });
          break;
      }
    ...
    addCountry: (newCountry) => {
      ...
      const pattern = /^[가-힣]+$/;
      if (!pattern.test(newCountry.countryName)) {
        alert("한글국가명만 입력");
        return false;
      }
      ...
      localStorage.setItem(0, JSON.stringify(tempConList));
      alert("추가완료");
      return true;
    },
    ...
    removeCountry: (targetCountryName) => {
      const tempCountry = countryList.filter((country) => {
        return country.countryName !== targetCountryName;
      });
      setConList(tempCountry);
      localStorage.setItem(0, JSON.stringify(tempCountry));
    },
